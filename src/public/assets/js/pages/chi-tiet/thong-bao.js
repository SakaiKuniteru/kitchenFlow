...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function bindEvents() {
        elements.filters.forEach(button => {
            button.addEventListener(
                ...
                () => {
                    state.filter =
                        button.dataset.notificationPageFilter ||
                        "all";
    ...
    function render() {
        ...
        elements.filters.forEach(button => {
            button.classList.toggle(
                "is-active",
                button.dataset.notificationPageFilter === state.filter
            );
        });
    ...
    function renderItem(item) {
        ...
        const previewContent = stripHtml(
            item?.noiDung ||
            ""
        );

        const richContent = sanitizeRichTextHtml(
            item?.noiDung ||
            ""
        );
    ...
    function getCreatorName(item) {
        return (
            item?.nguoiTao?.nhanVien?.hoTen ||
            item?.nguoiTao?.tenDangNhap ||
            "Hệ thống"
        );
    }
    ...
    function hideNoPermission() {
        ...
        const noPermission =
            pageContent
                ?.querySelector(":scope > [data-catalog-no-permission]") ||
            document.querySelector("[data-catalog-no-permission]");
                ...
                .forEach(node => {
                    ...
                    Array.from(node.attributes)
                        .forEach(attribute => {
                            ...
                            if (name === "style") {
                                const safeStyle = sanitizeRichTextStyle(
                                    tag,
                                    attribute.value
                                );
                    ...
                    if (tag === "A") {
                        const href = normalizeRichTextUrl(
