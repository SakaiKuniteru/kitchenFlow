...
async function docDuLieuImport(file) {
    ...
    for (let rowNumber = DATA_START_ROW; rowNumber <= worksheet.rowCount; rowNumber++) {
        ...
        if (dsNvQuanLyId !== undefined) {
            item.dsNvQuanLyId = parseDanhSach(dsNvQuanLyId)
...
function validateDongImport(item) {
    ...
    if (item.dsNvQuanLyId !== undefined) {
        const idKhongHopLe = item.dsNvQuanLyId.some(
            id =>
...
async function taoDuLieuNghiepVu(item) {
    ...
    if (item.dsNvQuanLyId !== undefined) {
        const dsNhanVien = await nhaAnRepository.getDsNhanVienByIds(
            item.dsNvQuanLyId
        );
    ...
    if (item.dsMaNvQuanLy !== undefined) {
        ...
        if (dsNhanVien.length !== item.dsMaNvQuanLy.length) {
            throw new ApiError(
                400,
                "Danh sách mã nhân viên quản lý không hợp lệ."
            );
        }

        const dsIdTheoMa = dsNhanVien.map(
            item => Number(item.id)
        );
        ...
        if (item.dsNvQuanLyId !== undefined) {
            ...
            if (
                JSON.stringify(dsIdTruyen) !==
                JSON.stringify(dsIdTheoMaSapXep)
            ) {
                throw new ApiError(
                    400,
                    "Danh sách ID và mã nhân viên quản lý không khớp."
                );
            }
        }
    ...
    for (const item of danhSach) {
        try {
            ...
            if (xuLy.action === "UPDATE") {
                ...
                const result = await nhaAnService.update(
                    xuLy.record.id,
                    data
                );
            ...
            const result = await nhaAnService.create(
                data
            );
